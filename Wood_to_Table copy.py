...
def run(_context: str):
    try:        
        ...
        if not qty_input[1]:  # Si el usuario no cancela
            ...
            unit = design.fusionUnitsManager.defaultLengthUnits
            unit_symbol = unit
            ...
            def get_body_dimensions(body, units_manager, target_unit):
                ...
                dim_x = round(units_manager.convert(dim_x_cm, 'cm', target_unit), 2)
                dim_y = round(units_manager.convert(dim_y_cm, 'cm', target_unit), 2)
                dim_z = round(units_manager.convert(dim_z_cm, 'cm', target_unit), 2)
